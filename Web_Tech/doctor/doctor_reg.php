...
if(isset($_POST['signup'])) {
    ...
    if($password == $confirm_password){
        ...
        if(!$result->num_rows > 0) {
            $sql = "INSERT INTO doctor(d_FullName, d_email, d_number,d_specialize, d_address, d_password, d_DOB, d_gender) VALUES (?,?,?,?,?,?,?,?)";
            $stmtinsert = $conn->prepare($sql);
            $result = $stmtinsert->execute([$dFullName, $dEmail, $dNumber,$dcomments, $dAddress, $password, $DOB, $dGender]);
            if($result){
