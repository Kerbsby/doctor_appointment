...
if(isset($_POST['signup'])) {
    ...
    if($password == $confirm_password){
        ...
        if(!$result->num_rows > 0) {
            $sql = "INSERT INTO patient(p_FullName, p_email, p_number, p_address, p_password, p_DOB, p_gender) VALUES (?,?,?,?,?,?,?)";
            $stmtinsert = $conn->prepare($sql);
            $result = $stmtinsert->execute([$pFullName, $pEmail, $pNumber, $pAddress, $password, $DOB, $pGender]);
            if($result){
