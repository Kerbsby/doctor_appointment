...
if(isset($_POST['signup'])) {
    ...
    if($password == $confirm_password){
        ...
        if(!$result->num_rows > 0) {
            $sql = "INSERT INTO patient(p_FirstName, p_LastName, p_email, p_number, p_address, p_password, p_DOB, p_gender) VALUES (?,?,?,?,?,?,?,?)";
            $stmtinsert = $conn->prepare($sql);
            $result = $stmtinsert->execute([$pFirstName, $pLastName, $pEmail, $pNumber, $pAddress, $password, $DOB, $pGender]);
            if($result){
        ...
        <form action="register.php" class="form-signup border shadow p-3 rounded" method="post">
            ...
            <div class="row">
                <div class="col-sm-6 col-sm-6">
                    <label>First Name</label><input type="text" name="pFirstName" value="" id="firstname" class="form-control input-lg" placeholder="First Name" required />
                </div>
                <div class="col-sm-6 col-sm-6">
                    <label>LastName</label><input type="text" name="pLastName" value="" id="lastname" class="form-control input-lg" placeholder="Last Name" required />
                </div>
            </div>
            <label>Email</label>
